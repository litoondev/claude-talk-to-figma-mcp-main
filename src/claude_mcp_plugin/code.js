...
/**
 * Scale a section's padding toward a breakpoint.
 *
 * Merged behaviour: the recursion and variable-binding checks come from the
 * local responsive work, the `verticalPadding` cap from upstream. `depth` is
 * internal to the recursion and stays last so the call site keeps upstream's
 * four-argument shape.
 */
function applyPaddingScale(node, factor, sidePadding, verticalPadding, depth) {
  if (!isAutoLayout(node)) return;
  // An absolutely-positioned layer is a deliberate composition. Its spacing is
  // not ours to rescale, and neither is anything inside it.
  ...
  if (depth === 0) {
    ...
    // Vertical padding is capped at the breakpoint's section minimum when one
    // is given, and scaled otherwise — bound values are left to their variable.
  ...
  if (isContainer(node)) {
    for (const c of node.children) {
      applyPaddingScale(c, factor, sidePadding, verticalPadding, depth + 1);
    }
  }
}

/**
 * Scale the gaps in a subtree toward a breakpoint.
 *
 * Merged behaviour: the recursion and the variable-binding check come from the
 * local responsive work; `maxGap` is upstream's rule that no gap may exceed the
 * breakpoint's side padding. A gap bound to a variable is left to its variable.
 */
function applyGapScale(node, factor, maxGap) {
  if (!isAutoLayout(node)) return;
  // Same rule as applyPaddingScale: absolute layers are left exactly as drawn.
  if (isAbsolutePositionedLayer(node)) return;
  if (typeof node.itemSpacing === "number" && !hasBoundVar(node, "itemSpacing")) {
    let gap = scaleSpacing(node.itemSpacing, factor, 8);
    if (typeof maxGap === "number") gap = Math.min(gap, maxGap);
    node.itemSpacing = gap;
  }
  if (isContainer(node)) {
    for (const c of node.children) {
      applyGapScale(c, factor, maxGap);
    }
  }
}
...
async function generateBreakpoint(source, presetKey, options) {
  ...
  // Restored by restoreTypography() once the responsive pass is done.
    ...
    for (const behavior of behaviors) {
      try {
        if (behavior === "reduce-padding") {
          ...
          applyGapScale(child, Math.max(0.5, factor), preset.sidePadding);
...
async function setMultipleTextContents(params) {
  ...
  for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
    ...
    // Yield between chunks so Figma's UI stays responsive. The old full second
    // was there to let the orange highlight animation play out; that highlight
    // is gone, so a short yield is enough and a large job no longer crawls.

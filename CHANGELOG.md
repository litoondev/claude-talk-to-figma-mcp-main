...
### Added
- **🧹 Audit of the whole Webflow integration, after the documentation fell behind the code**: the readme, CLAUDE.md and a tool description still described a state that stopped being true when this plugin grew its own Webflow tools.
  - **The readme still told users to install Webflow's connector.** Its setup section opened "This plugin has no Webflow tools" and walked through Add connectors, the `mcp-remote` shim and Webflow's MCP Bridge App — the exact instructions that made a session report it could not build layout. Replaced in both the English and Bengali guides with the two switches that actually matter (API token for content, the Designer tools checkbox for canvas), the restart requirement, `npm run webflow:check`, `npm run webflow:harness`, and a note that being asked to install Webflow's connector *means the checkbox is off*.
  - **CLAUDE.md §17.5 said canvas work "runs through Webflow's own MCP server until the Designer Extension exists".** It exists. Rewritten as a table of the two halves and their switches, stating plainly that Webflow's server, connector and Bridge App are neither used nor needed.
  - **The documented tool count had drifted to 162 against a real 167.** That number is the entire justification for the profile system — the schema is re-sent on every request — and it was being maintained by hand and quoted onward into the readme as fact. `tests/unit/tool-counts.test.ts` now counts the live registry and fails when the header disagrees, checks the `full` line matches the header, and catches duplicate registrations, which silently overwrite each other.
  - **`uuid` is ESM-only and ts-jest cannot parse it**, so any test reaching `utils/websocket.ts` failed on an import rather than on its subject — which is why the preflight tests had to mock the whole relay module. Mapped to a small stub in `jest.config.cjs`, so tests can now load the real tool registry.

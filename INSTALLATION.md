...
## 4. Optional: enable comment tools (Figma REST token)

The comment tools (`get_my_comments`, `reply_to_comment`, `reply_to_comments`, …) are the **only** tools in this project that do not go through the WebSocket relay.

**Why:** Figma's *Plugin* API cannot see comments at all — comments are not part of the document node tree and are never exposed to plugins. They exist only in Figma's REST API, which authenticates with a personal access token instead of a plugin channel.

Consequences:

- These tools need a token; the other ~120 tools do not.
- These tools work **without** the socket running and **without** `join_channel`.
- Every other tool continues to work with no token configured.

### Create a token

1. In Figma: **Account settings → Security → Personal access tokens → Generate new token**
2. Grant at least:
   - `files:read` — read files, projects and comments
   - `file_comments:write` — post and delete replies
3. Copy the token (shown once). It looks like `figd_...`.

### Expose it to the MCP server

Add an `env` block to whichever config you already use.

**Claude Desktop / Cursor / Windsurf / Cline / Roo Code (JSON):**
...
which calls `get_figma_account` and echoes back your user id and handle.

### Optional tuning

| Variable | Default | Purpose |
|----------|---------|---------|
| `FIGMA_ACCESS_TOKEN` | — | Personal access token. Required for comment tools. |
| `FIGMA_PERSONAL_ACCESS_TOKEN` | — | Accepted as a fallback name. |
| `FIGMA_API_CONCURRENCY` | `4` | Files fetched in parallel during a sweep. Lower it if you hit rate limits. |
| `FIGMA_API_MAX_RETRIES` | `3` | Retries on `429` / `5xx` / network errors. |
| `FIGMA_API_TIMEOUT_MS` | `30000` | Per-request timeout. |
| `FIGMA_API_BASE_URL` | `https://api.figma.com` | Override for testing or a proxy. |

### Finding ids

- **File key** — from the file URL: `figma.com/design/<FILE_KEY>/<name>`
- **Team id** — from the team URL: `figma.com/files/team/<TEAM_ID>/...`
- **Project ids** — run `list_figma_files` with your `teamId`

> **Security note:** the token grants read access to every file your account can open. Keep it in the client's `env` block (or your OS keychain / secret manager), never in a committed file. Prefer a token scoped only to `files:read` if you never intend to post replies.
